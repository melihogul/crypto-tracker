...
export const columns: ColumnDef<AssetsType>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const price = data.price_change_percentage_24h
        .toLocaleString()
        .startsWith("-")
        ? parseFloat(
            data.price_change_percentage_24h.toString().substring(1)
          ).toFixed(2)
  ...
  },
  {
    id: "holdings",
    header: () => {
      return <div className="select-none text-right">Holdings</div>;
    },
    cell: ({ row }) => {
      const buyQuantity = row.original.auditLogs
        .filter((item) => item.action === "BUY")
        .map((item) => item.quantity)
        .reduce((accumulator, currentValue) => accumulator + currentValue, 0);
      const sellQuantity = row.original.auditLogs
        .filter((item) => item.action === "SELL")
        .map((item) => item.quantity)
        .reduce((accumulator, currentValue) => accumulator + currentValue, 0);
      const holdings = buyQuantity - sellQuantity;
      const holdingsWorth = parseFloat(
        (holdings * row.original.current_price).toFixed(2)
      );
  ...
  },
  {
    ...
    header: () => {
      return (
        <div className="select-none cursor-pointer text-right">
          Avg. Buy Price
        </div>
      );
    },
    cell: ({ row }) => {
      ...
      const buyLogs = auditLogs.filter((item) => item.action === "BUY");
      ...
      const avgBuyPrice = totalAmount > 0 ? totalPrice / totalAmount : 0;
  ...
  },
  {
    id: "profit",
    header: () => {
      return <div className="select-none text-right">Profit/Loss</div>;
    },
    cell: ({ row }) => {
      const auditLogs = row.original.auditLogs;
      ...
      const buyQuantities = auditLogs
        .filter((item) => item.action === "BUY")
        .map((item) => item.quantity);
      const buyQuantity =
        buyQuantities.length > 0
          ? buyQuantities.reduce((acc, val) => acc + val)
          : 0;

      const sellQuantities = auditLogs
        .filter((item) => item.action === "SELL")
        .map((item) => item.quantity);
      const sellQuantity =
        sellQuantities.length > 0
          ? sellQuantities.reduce((acc, val) => acc + val)
          : 0;

      const holdings = buyQuantity - sellQuantity;
      const holdingsWorth = parseFloat((holdings * currentPrice).toFixed(2));

      const buyPrices = auditLogs
        .filter((item) => item.action === "BUY")
        .map((item) => item.price * item.quantity);
      const totalCost =
        buyPrices.length > 0 ? buyPrices.reduce((sum, item) => sum + item) : 0;

      const profitPercentage =
        totalCost > 0 ? ((holdingsWorth - totalCost) / totalCost) * 100 : 0;
